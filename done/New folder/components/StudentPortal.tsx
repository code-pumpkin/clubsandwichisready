...
import { Home, Book, Calendar, DollarSign, FileText, MessageSquare, Award, Video, User, Menu, X } from 'lucide-react';
...
const StudentPortal: React.FC<StudentPortalProps> = ({ email, onLogout }) => {
  ...
  const renderContent = () => {
    const title = menuItems.find(item => item.id === activeSection)?.label || 'Unknown Section';
